...
impl AgentPlugin {
    ...
    /// # Panics
    ///
    /// If the adapter's instance lock is poisoned — a previous call panicked
    /// mid-invocation, so the instance state is unknown and every later call
    /// stays poisoned rather than running on it.
