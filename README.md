...
Desktop app and CLI: **[Releases](https://github.com/GlimpseEngine/token-station/releases/latest)**.

- **macOS (Apple Silicon)** — `token-station_1.1.0_aarch64.dmg`. This build is **not yet Apple-signed**, so on first launch macOS Gatekeeper blocks it. To open: right-click the app → **Open** → **Open**, or run once: `xattr -dr com.apple.quarantine /Applications/token-station.app`. Intel and signed builds are coming in a later patch.
