...
[![Release](https://img.shields.io/github/v/release/GlimpseEngine/token-station?display_name=tag&sort=semver)](https://github.com/GlimpseEngine/token-station/releases/latest) [![CI](https://github.com/GlimpseEngine/token-station/actions/workflows/ci.yml/badge.svg)](https://github.com/GlimpseEngine/token-station/actions/workflows/ci.yml) [![License](https://img.shields.io/github/license/GlimpseEngine/token-station)](LICENSE)

[Download](https://github.com/GlimpseEngine/token-station/releases/latest) · [Quick start](#quick-start) · [Documentation](docs/README.md) · [Report an issue](https://github.com/GlimpseEngine/token-station/issues) · [简体中文](README.zh-CN.md)
</div>
...
Download a published build from **[GitHub Releases](https://github.com/GlimpseEngine/token-station/releases/latest)**, or build the current source tree yourself.
...
If a release provides an unsigned or unnotarized macOS test DMG, verify its published SHA-256 and follow the [macOS installation notice](docs/release/macOS-DMG安装提示与打包要求.md). Do not disable Gatekeeper system-wide. Build locally if you need an artifact derived from source you inspected. If you require a Developer ID signed and notarized binary, use only a release that explicitly provides one.
...
- [User documentation](docs/product/README.md) (Chinese)
- [Routing model](docs/product/路由机制.md) (Chinese)
- [Configuration reference](docs/product/配置详解.md) (Chinese)
- [CLI command reference](docs/product/CLI-命令参考.md) (Chinese)
- [Agent setup guides](docs/guides/) (Chinese)
- [Architecture overview](docs/contributing/架构总览.md) (Chinese)
- [Development environment](docs/contributing/开发环境.md) (Chinese)
- [Testing and contribution guide](docs/contributing/README.md) (Chinese)

## Contributing

Issues and focused pull requests are welcome. Read the [contribution guide](docs/contributing/贡献流程.md) first. User-visible UI, interaction, state, contract, or release behavior changes require a design document in `docs/design/` before tests and implementation; keep that document in the same pull request as the change.
