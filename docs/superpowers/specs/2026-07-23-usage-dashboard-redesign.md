...
| Output | `#25a86f` | 输出 token |
| Cache write | `#ef8a2f` | 缓存写入 |
| Cache read | `#8b5cf6` | 缓存命中 |
| Cost | `#ef476f` | 成本虚线 |
...
├─────────────────────────────────────────────────────────┤
│ 请求日志                    [全部状态]        1–20 / 170 │
│ 时间  Agent  路由  状态  Token  延迟  成本              │
│ ▸ 展开：请求 ID / Token 明细 / 尝试 / 协议转换          │
├─────────────────────────────────────────────────────────┤
│ 预算状态                                                 │
│ ▸ 预算与定价管理                                        │
└─────────────────────────────────────────────────────────┘
...
- `24h` 固定补齐连续 24 个小时槽；`7d/30d` 分别补齐连续 7/30 个自然日。没有请求的桶保留为空槽，不再把三个有数据的小时等距铺满整个横轴。
- 横轴最多显示 6 个关键时间刻度，并在跨日处显示日期；左纵轴显示 Token，右纵轴显示成本，单位明确为“每小时”或“每天”。
- 图表上方直接展示“活跃桶数 / 总桶数”、Token 单桶峰值和成本单桶峰值，避免把纵轴峰值误认为总量。
- 参考 cc-Switch 的信息架构，在同一图表展示输入、输出、缓存写入、缓存命中 4 条平滑 Token 面积曲线；各序列独立呈现，不堆叠、不把缓存子集重复计入总 Token。
- 成本使用右轴红色虚线；有请求但没有定价的桶显示为断点和“成本未知”，不得画成零成本。
- 每个非空数据桶支持鼠标悬停与键盘聚焦；十字线和统一 Tooltip 同时显示时间、请求数、输入、输出、缓存写入、缓存命中与成本。
- 无数据时保留坐标区域并给出下一步，而不是隐藏整个模块。
- SVG 图表提供 `role="img"` 与摘要 `aria-label`；数据桶通过 `<title>` 和完整 `aria-label` 提供精确值。
- 趋势、明细和预算不是顺序流程，标题前不使用 `01/02/03` 装饰编号。
...
参考 cc-Switch 的成本链路，但沿用 Token Station 的版本化 Receipt 审计：

1. 若上游协议明确返回实际费用，记录为 `actual`。
2. 否则按请求最终路由模型匹配版本化价格表，记录为 `estimated` 和对应 `price_version`。
3. 内置价格表覆盖常用 Claude、OpenAI、DeepSeek、MiniMax、GLM 模型；模型 ID 匹配需兼容供应商命名空间、大小写、`.` / `-`、日期与 reasoning effort 后缀。
4. 新增内置价格或用户补充价格时，只回填 `cost_kind=unknown` 且已有 Token 的历史 Receipt；已有实际或估算成本永不重估。
5. 未匹配模型仍显示“成本未知”，并在请求日志中提示缺少哪个模型的价格，不允许当作零成本。

Token Station 的规范化 `Usage` 中，缓存读写属于输入子集，推理属于输出子集。估算公式为：

```text
fresh_input  = input - cache_read - cache_write
fresh_output = output - reasoning

cost = fresh_input × input_price
     + cache_read × cache_read_price
     + cache_write × cache_write_price
     + fresh_output × output_price
     + reasoning × reasoning_price（缺省使用 output_price）
```

所有价格单位为 USD / 1M Token，数据库存储为微美元。

### 6.7 请求日志

- 用量页展示完整本地 Receipt 日志，默认每页 20 条，按时间倒序。
- 顶部 Agent、供应商、模型和时间范围筛选同时作用于请求日志；日志区另提供全部/成功/失败状态筛选。
- 摘要列包含时间、Agent、最终路由、HTTP 状态、总 Token、延迟和成本类型；成本明确区分实际、估算、未知。
- 展开行显示请求 ID、请求模型、输入/输出/缓存/推理 Token、价格版本、路由决策、上游尝试和协议转换。
- 请求正文和响应正文始终不采集、不展示。
- 分页由 SQLite 查询完成，不把完整历史一次性加载到前端。
