# Claude Code Anthropic 入站适配与 DeepSeek 接入设计

## 1. 目标

在不修改 Claude Code 自身逻辑的前提下，使其通过 `ANTHROPIC_BASE_URL`
连接本地 token-station，由新增的 `agent-anthropic` 将 Anthropic Messages
请求归一为 Canonical IR，经现有路由器选择 DeepSeek 模型，再由现有
`provider-openai-compatible` 调用 DeepSeek OpenAI-compatible API。返回的
`ChatResponse` / `StreamEvent` 重新渲染为 Anthropic Messages 响应。

本设计覆盖需求文档的 M0–M3：环境基线、Anthropic 非流式适配、
流式/工具调用/Hint，以及 Claude Code 端到端验证。其他 Agent 的扩展
不在本轮实现范围。
...
不采用将 Anthropic 请求透传到 `https://api.deepseek.com/anthropic` 的快速方案，
...
新增 `plugins/official/agent-anthropic` 并完整实现 `agent-adapter-v1`：
...
- Anthropic `image.source` 中的 URL 或 base64 转为 `ImageUrl`；base64 使用
  data URL 表示，避免改动 IR。DeepSeek 当前不支持图片属于已知上游限制。
...
- `thinking` / `redacted_thinking` 目前无完整 IR 表示。首版接入文档显式
  关闭 Claude Code adaptive thinking 与实验 beta，不伪造无损支持；后续以 IR
  变更请求处理。
...
模型名只存在配置中，不硬编码到 adapter。路由器把 Claude Code 请求的
Claude 模型名映射到配置池中的 DeepSeek 模型。
...
- 相关 CLI/plugin registry 测试或发布物料引用
- 不含凭证的 DeepSeek 示例配置
- Claude Code 接入文档
...
1. 设计规格。
2. `agent-anthropic` 非流式实现与 normalize/render/error fixtures。
3. 流式、工具调用、Hint 与剩余 fixtures。
4. `/v1/messages` 宿主集成、DeepSeek 配置和接入文档。
5. 端到端验收修正。
