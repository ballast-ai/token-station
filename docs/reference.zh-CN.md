...
Codex 实时 Web Search 需要上游实现原生 OpenAI Responses API。请给明确选定的 `provider: openai-compatible` 上游设置 `"api_dialect": "responses-native"`，并让 `base_url` 结束于 API 版本段，例如 `https://api.openai.com/v1`。此后，声明了有效 `web_search` 工具的 Responses 请求会转发到 `base_url` + `/responses`；Token Station 会保留请求与响应格式，包括 `web_search_call` JSON 项和 SSE 事件，只改写 `model`。未启用 Web Search 的请求遵循已配置的路由策略。图片请求（包括历史中保留图片的续接）在选中 `responses-native` 通道时使用 Responses 接口，其他请求使用转换路径。原生续接会重放有容量限制的本地历史，不向另一个账户发送原账户的响应 ID。翻译型上游仍会在发起网络请求前拒绝由供应商执行的工具。桌面端目前不提供这个高级设置。
...
已配置的账户套餐在各种路由模式下共同累计本地消费。私有 `quota.sqlite` 按账户和兼容的套餐配置保存计数，网关重建后恢复，不会重放请求。预检另一套套餐不会替换正在运行的套餐历史。

额度页区分新建、已恢复、套餐变更和不可用的历史，并单独提示缺失 Token 用量的请求数。Token Station 之外的请求，以及其他套餐配置下的消费，不包含在当前套餐估算中。供应商后续响应不再携带额度头时，旧的供应商额度快照会失效。

额度持久化失败时会明确报错，不会假装余额已恢复。应先从可信备份恢复 `quota.sqlite`，再移除 `quota-state-unavailable` 标记；不要仅删除标记就把余额当成已恢复。
...
| 桌面端请求正文 | 桌面网关默认把客户端输入、供应商交互和最终输出保存为仅当前用户可读的明文 JSON，供“请求日志”查看。默认保留阈值为 7 天，目标上限为 1000 个请求文件。单个正文上限为 256 KiB，单次 HTTP 跟踪正文合计上限为 1 MiB。请求头名称会被记录，只有诊断安全白名单中的头值会保留，其余值显示为 `<redacted>`。Host 注入的供应商凭证不会被捕获，正文中的 Secret 仍会随正文保留。启动时、写入期间以及网关准备或运行期间每小时执行清理，日志界面拒绝读取已过期正文。 |
...
模型价格支持 `cache_write_5m_per_mtok` 和 `cache_write_1h_per_mtok`，单位为每百万 Token 的微美元。缺失的专属价格以及没有 TTL 明细的用量均使用 `cache_write_per_mtok`。历史收据保留记录时的成本和价格版本。TTL 费率不同时，Agent 元数据不会把它们伪装成统一的缓存写入单价。
