...
| 本地客户端（社区版，个人 BYOK 模式的壳，开核开源） | 🟡 | 本地规则引擎+内嵌小分类器+直连官方 API。**归属已拍板（2026-07-06）：由 token-station 开发交付**；glm5.2-platform 为被集成方之一（OAuth 设备码授权 + 只读 API），本地对账工具为其内置功能。2026-07-08 二次修订：无本地 Web 界面（界面包取消），指标库随本体默认开启；四次修订：当前范围为 BYOK / 本地模型 / 平台账户三类上游。**2026-07-09 修订：开核开源（Apache-2.0）取代原闭源注册下载口径**——任意下载、可自行构建，注册解锁云功能；信任验证以源码公开 + 可复现构建为主，网络边界、字段白名单、本地审计与离线可用降为纵深验收。完整需求见 [个人模式本地客户端需求](./个人模式本地客户端需求.md) | OmniRoute 形态 |
