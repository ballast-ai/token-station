...
2. **威胁上调（另一方向）**：cc-switch README 里 29 家中转商的赞助位是一门**真实且被市场定价**的流量生意。真正值得警惕的不是这一家，而是「**11.5 万 star 的本地客户端 = 中转站行业的获客入口**」这个结构。

> 🔄 **2026-07-09 五次修订后更新**：本文初稿在此处称「token-station 的『注册前置 + 闭源分发』等于主动放弃这个位置」。客户端口径已改为 **Apache-2.0 开核开源、任意下载、注册后移到云功能**，**该论断作废**——我们有资格占据这个入口，且逻辑与 cc-switch 不同：它把流量卖给中转商赚返佣，我们把流量导向自己的平台端点。详见 [竞品全景 §五](./AI编码工具配置切换器竞品全景.md)。
...
> 🔄 **2026-07-09 五次修订后改写**。初稿在此处写道：「token-station 的『注册前置 + 闭源分发』等于主动放弃这个位置，而对手可以用返佣买下它」，并建议回到 [个人模式本地客户端需求 §五](../features/个人模式本地客户端需求.md) 重新掂量这笔「第二笔代价」。
>
> **该建议已被采纳并超前完成**：五次修订把客户端改为 Apache-2.0 开核开源、任意下载、注册后移到云功能。入口位置不再被放弃。

改写后的结论：入口的价值由这 29 家赞助商的付费行为**独立验证**——本地客户端到付费上游的转化是真实的、可定价的。区别在于变现方式：cc-switch 把入口**出租**给中转商（返佣），本产品把入口**自用**（导向平台端点，注册解锁云功能）。同一个入口，两种变现。
...
token-station 的信任验收机制若做实并讲清楚，**可以直接对比出优势**。按 [2026-07-09 五次修订](../features/个人模式本地客户端需求.md)，其主验证手段已从「闭源下的信任验收」改为**源码公开 + 可复现构建**——「同步字段白名单结构性不可达」从一句承诺变成任何人可读源码核实的事实；网络边界可查、离线可用、本地审计输出降为纵深防御。

**这个改动恰好把我们和 TeamoRouter 放到了叙事的两端**：它的一切都靠自报（99.6% SLA、90% 折扣、企业功能），而我们的核心主张变成可被第三方独立复现。
