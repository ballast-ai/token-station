...
**推论**：本产品相对本品类的护城河，几乎全部来自 **token-station 平台侧的存在**（平台账户上游 + 独立对账），而非本地客户端本身。

> 🔄 **2026-07-09 五次修订后更新**：客户端口径已改为**开核开源（Apache-2.0）、任意下载、注册后移到云功能**（见 [个人模式本地客户端需求 §五](../features/个人模式本地客户端需求.md)）。本文初稿曾判定「本地客户端单独与 CCR / CCS 对比是劣势方（闭源、注册下载 vs MIT、匿名安装）」——**该劣势已解除**：许可与分发门槛拉平，竞争回到功能差异本身。初稿另记的「闭源的第二笔代价：放弃入口位置」亦随之作废，改写见 §五。
...
|---|---|---|
| 账号体系 | opt-in（默认无账号、纯本地） | opt-in：纯本地功能零账号，云功能解锁时注册 🔄 |
...
| 许可 / 分发 | MIT、匿名安装 | Apache-2.0 开核开源、任意下载 🔄 |

> 🔄 = 按 2026-07-09 五次修订更新。初稿此两行为「注册前置（下载即注册）」与「闭源、注册下载」。

**它的隐私叙事与我们几乎一字不差。** 初稿据此判定「它是 MIT 且零门槛，对人群 2 构成获客摩擦」——**开核开源拍板后，这层摩擦解除**：双方同为开源、同为零门槛，纯本地功能均不需账号。竞争回到功能差异本身，而我们在功能上是占优的一方（见下段）。
...
> 🔄 **2026-07-09 五次修订后改写**。本文初稿在此处写道：「本产品『注册前置 + 闭源分发』等于主动放弃这个入口位置，而竞争对手可以用返佣把它买下来」，并称之为「闭源的第二笔代价」。**该论断已作废**——客户端改为 Apache-2.0 开核开源、任意下载、注册后移到云功能（Tailscale 模式）后，本产品**有资格占据这个入口位置**。

改写后的结论：

- 入口的价值被 cc-switch 的 29 家赞助商**独立验证**了——中转商愿意付费买它，说明本地客户端到付费上游的转化是真实的；
- 但本产品与 cc-switch 的入口逻辑**不同**：cc-switch 把流量**卖给**中转商赚返佣，我们是把流量**导向自己的平台端点**。我们不是入口的出租方，是入口的自用方；
- 这恰好就是五次修订「注册解锁云功能」的商业机理：本地功能免费开源做大漏斗，平台账户上游 / 云同步 / 多设备报表 / 对账作为注册钩子。**它与 cc-switch 的返佣模式是同一个入口，两种变现**。

**遗留判断**：我们不赚返佣，就意味着漏斗顶端的绝对流量打不过一个愿意收 29 家赞助费的项目。开源解除了门槛劣势，但没有解除**规模劣势**——11.5 万 star 不是靠开源license 得来的，是靠覆盖 7 款工具的一站式配置面。这一点在 §三 已有结论。
...
2. **cc-switch 生态长出形态分身**：`cc-switch-cli`（4,074★，第三方 fork）与 `cc-switch-web`（470★）。一个 GUI 项目被复刻成 CLI 与 Web，说明**管理面形态是用户偏好而非产品本质**——这反过来支持本产品「命令行唯一本地管理面」的拍板：形态可以后补，数据面先立住。

   > 🔄 **2026-07-09 五次修订后强化**。初稿在此处提示「若本产品闭源，别人无法这样为你补形态」。开核开源（Apache-2.0）拍板后，**这条从风险变成了资产**：社区可以为我们补 GUI / Web / 托盘形态，就像他们为 cc-switch 补了 CLI 与 Web 一样。「命令行唯一本地管理面」的拍板因此代价更低——我们不做的形态，社区可以做，而且做出来仍然跑我们的路由内核。
