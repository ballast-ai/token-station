...
| 阶段 | 内容 | 依赖 | 粗估 |
|------|------|------|------|
| **C1 最小可用** | 本地端点（127.0.0.1，OpenAI 兼容+流式）+ BYOK 直连 + 本地模型（Ollama/vLLM）+ 规则/启发式路由（复用 P1 路由 crate）+ 密钥保管（钥匙串/主密码）+ 文件日志 + 指标库（SQLite，默认开启）+ CLI 管理面 + 简统计命令 + 开源首发（任意下载 + 可复现构建 + 签名发布，零绑定可用，2026-07-09 开核口径）+ 离线可用与网络边界最小化验收 | P1 路由 crate 骨架；官方包签名分发可先用 V1 现有基础设施 | 9 人周（2026-07-09：+1 可复现构建与发布工程） |
...
**客户端小计：≈ 18 人周**（语言拍板 Rust，见 [升级方案 §五#6](../architecture/cloud-ai-gateway升级差异分析与业务方案.md)；若拍 Go 则路由内核双实现，C1 +3 人周且长期双维护）
...
| 客户端 C1–C3 | ≈ 18 |
| **合计** | **≈ 93 人周**（含 ~20% 各期内置的测试与文档，不含产品/设计/运营）|
...
| C1 | IDE/Agent 指向 `127.0.0.1` 可用；BYOK / 本地模型混合路由；断网可用；key 不进日志；指标库默认开启；关闭云同步/远程配置时外联目的地可解释；官方二进制可由公开源码复现构建（开源首发前提） | 个人版（社区版）本地试用 + 开源首发 |
...
| [个人版开发计划](个人版开发计划.md) | 客户端支线 C0–C3 的独立细化计划；C1 含可复现构建与开源首发（+1 人周已回写本文）；开核开源口径（Apache-2.0、注册解锁云功能）以其 §一为准 | 一致（2026-07-09）|
