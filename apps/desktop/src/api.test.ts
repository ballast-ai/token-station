...
describe("desktop API mapping and read-only HTTP data plane", () => {
  ...
  it.each([
    ...
    ["edit provider experimental runtime", () => editProvider(
      "p",
      "https://p/v1",
      null,
      "env",
      ...
      providerCall: "south_v1_buffered_streaming",
    }],
    ["edit provider South Header Auth engine", () => editProvider("p", "https://p/v1", null, "env", "P_KEY", "south_v1_buffered_streaming_header_auth"), "edit_provider_with_credential", {
      name: "p",
      baseUrl: "https://p/v1",
      apiKey: null,
      credentialSource: "env",
      credentialReference: "P_KEY",
      providerCall: "south_v1_buffered_streaming_header_auth",
    }],
