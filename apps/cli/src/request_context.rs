...
mod tests {
    ...
    fn a_cancelled_client_cancels_the_context() {
        let ctx = RequestContext::detached(Duration::from_secs(60), Duration::from_secs(30));
    ...
    fn a_drain_cascades_into_live_contexts() {
        ...
        let ctx = RequestContext::new(&drain, Duration::from_secs(60), Duration::from_secs(30));
        assert!(!ctx.is_cancelled());
        drain.cancel();
        assert!(ctx.is_cancelled());
    }
