...
assert.match(entry, /if \[\[ "\$version" != "2\.0\.0" && "\$version" != "2\.1\.1" && "\$version" != "2\.1\.2" \]\]/);
...
assert.match(releaseWorkflow, /needs\.release-target\.outputs\.tag != 'v2\.0\.0' && needs\.release-target\.outputs\.tag != 'v2\.1\.1' && needs\.release-target\.outputs\.tag != 'v2\.1\.2'/);
